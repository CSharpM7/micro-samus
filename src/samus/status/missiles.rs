...
unsafe fn missile_homing_main(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    weapon.global_table[SUB_STATUS].assign(&L2CValue::Ptr(missile_homing_substatus as *const () as _));
...
unsafe extern "C" fn missile_homing_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let despawn = WorkModule::count_down_int(weapon.module_accessor, *WEAPON_INSTANCE_WORK_ID_INT_INIT_LIFE,0) != 0;
