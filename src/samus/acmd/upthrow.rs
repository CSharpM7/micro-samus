...
unsafe fn game_throwhi(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        ...
        opponent_boma = util::get_grabbed_opponent_boma(agent.module_accessor);
    ...
    if macros::is_excute(agent) {
        ...
        //ArticleModule::generate_article_enable(agent.module_accessor, weapon, false, -1);
...
unsafe fn sound_throwhi(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        /* 
        let mut is_ice = VarModule::is_flag(agent.battle_object, samus::instance::flag::ICE_MODE);
        if !is_ice {
